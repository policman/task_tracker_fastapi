...
from app.api.v1.schemas.batch import BatchFilter, BatchExportFilter, ReportRequestSchema, BatchStatistics, \
...
async def get_batch_with_products(
    ...
    if not batch_with_products:
        raise HTTPException(detail=f"Batch with ID {batch_id} not found", status_code=status.HTTP_404_NOT_FOUND)
...
async def get_batch_statistics(batch_id: int, session: AsyncSession = Depends(db_helper.session_getter)):
    repo = BatchRepository(session)
